...
public class AttachmentServiceImpl implements AttachmentService {
    ...
    public ResponseModel<String> upload(String folder, MultipartFile file) {
        ...
        boolean isValidExt = extension.equals(".jpg")
                || extension.equals(".jpeg")
                || extension.equals(".png");

        if (!isValidType || !isValidExt) {
            log.warn("FAILED  : Invalid file type or extension for: {}", originalName);
        ...
        try {
            repository.upload(folder, id,
                    file.getInputStream(),
                    file.getSize(),
                    file.getContentType());

            log.info("FINISH  : Uploaded file successfully. Assigned ID: {}", id);
        } catch (IOException e) {
            log.error("ERROR   : Failed to process file stream for: {}", originalName, e);
            throw new ApiException(
                    ResponseConstants.ResponseStatus.BAD_REQUEST,
